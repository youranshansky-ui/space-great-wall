<template>
  <div>
    <div class="flex items-center justify-between mb-8">
      <h1 class="text-2xl font-display font-bold text-hologram-ice">地月纪元章节</h1>
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="item in list" :key="item.id" class="border-b border-hologram-ice/[0.03] hover:bg-white/[0.01]">
            ...
            <td class="px-4 py-3 text-hologram-ice/60">{{ item.series === 'prologue' ? '地月纪元' : '星际联合' }}</td>
      ...
      <div class="w-full max-w-2xl max-h-[85vh] overflow-y-auto bg-deep-space-900 border border-hologram-ice/[0.1] rounded-2xl p-6 mx-4">
        ...
        <form @submit.prevent="save" class="space-y-4">
          <div>
            ...
            <select v-model="form.series" class="w-full px-4 py-2.5 rounded-lg bg-deep-space-800/60 border border-hologram-ice/[0.1] text-hologram-ice text-sm outline-none focus:border-stellar-gold/30 transition-all">
              <option value="prologue">地月纪元</option>
              <option value="alliance">星际联合</option>
            </select>
          </div>
          <div>
            <label class="block text-xs text-hologram-ice/30 mb-2">章节号</label>
...
const API = '/api/admin/story-chapters'
...
const form = ref({ series: 'prologue', chapter_num: 0, title: '', desc: '', content: '' })
...
function openCreate() {
  ...
  form.value = { series: 'prologue', chapter_num: 0, title: '', desc: '', content: '' }
  showModal.value = true
}

function openEdit(item) {
  editingItem.value = item
  form.value = { series: item.series || 'prologue', chapter_num: item.chapter_num || 0, title: item.title || '', desc: item.desc || '', content: item.content || '' }
  showModal.value = true
}
