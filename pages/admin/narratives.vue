<template>
  <div>
    <div class="flex items-center justify-between mb-8">
      <h1 class="text-2xl font-display font-bold text-hologram-ice">叙事画卷</h1>
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="item in list" :key="item.id" class="border-b border-hologram-ice/[0.03] hover:bg-white/[0.01]">
            ...
            <td class="px-4 py-3 text-hologram-ice/50 font-mono text-xs">{{ item.num }}</td>
            <td class="px-4 py-3 text-hologram-ice/60">{{ item.title }}</td>
            <td class="px-4 py-3">
              <img v-if="item.image" :src="item.image" class="w-10 h-10 object-cover rounded border border-hologram-ice/[0.06]" />
              <span v-else class="text-hologram-ice/20 text-xs">-</span>
            </td>
            <td class="px-4 py-3">
      ...
      <div class="w-full max-w-2xl max-h-[85vh] overflow-y-auto bg-deep-space-900 border border-hologram-ice/[0.1] rounded-2xl p-6 mx-4">
        ...
        <form @submit.prevent="save" class="space-y-4">
          ...
          <div>
            ...
            <input v-model.number="form.num" type="number" class="w-full px-4 py-2.5 rounded-lg bg-deep-space-800/60 border border-hologram-ice/[0.1] text-hologram-ice text-sm outline-none focus:border-stellar-gold/30 transition-all" />
          </div>
          <div>
          ...
          <div>
            <label class="block text-xs text-hologram-ice/30 mb-2">标语</label>
            <input v-model="form.slogan" type="text" class="w-full px-4 py-2.5 rounded-lg bg-deep-space-800/60 border border-hologram-ice/[0.1] text-hologram-ice text-sm outline-none focus:border-stellar-gold/30 transition-all" />
          </div>
          <div>
...
const API = '/api/admin/narratives'
...
const form = ref({ image: '', num: 0, title: '', desc: '', slogan: '', sort_order: 0 })
...
function openCreate() {
  ...
  form.value = { image: '', num: 0, title: '', desc: '', slogan: '', sort_order: 0 }
  showModal.value = true
}

function openEdit(item) {
  editingItem.value = item
  form.value = { image: item.image || '', num: item.num || 0, title: item.title || '', desc: item.desc || '', slogan: item.slogan || '', sort_order: item.sort_order || 0 }
  showModal.value = true
}
