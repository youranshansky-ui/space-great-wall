<template>
  <div>
    <div class="flex items-center justify-between mb-8">
      <h1 class="text-2xl font-display font-bold text-hologram-ice">城市管理</h1>
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="item in list" :key="item.id" class="border-b border-hologram-ice/[0.03] hover:bg-white/[0.01]">
            ...
            <td class="px-4 py-3 text-hologram-ice/50">{{ item.subtitle }}</td>
            <td class="px-4 py-3 text-hologram-ice/50">{{ item.type }}</td>
            <td class="px-4 py-3">
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60" @click.self="showModal = false">
      <div class="w-full max-w-2xl max-h-[85vh] overflow-y-auto bg-deep-space-900 border border-hologram-ice/[0.1] rounded-2xl p-6 mx-4">
        ...
        <form @submit.prevent="save" class="space-y-4">
          ...
          <div>
            <label class="block text-xs text-hologram-ice/30 mb-2">类型</label>
            <input v-model="form.type" type="text" class="w-full px-4 py-2.5 rounded-lg bg-deep-space-800/60 border border-hologram-ice/[0.1] text-hologram-ice text-sm outline-none focus:border-stellar-gold/30 transition-all" />
          </div>
          <div>
...
const form = ref({ name: '', subtitle: '', image: '', desc: '', type: '', sort_order: 0 })
...
function openCreate() {
  ...
  form.value = { name: '', subtitle: '', image: '', desc: '', type: '', sort_order: 0 }
  showModal.value = true
}

function openEdit(item) {
  editingItem.value = item
  form.value = { name: item.name || '', subtitle: item.subtitle || '', image: item.image || '', desc: item.desc || '', type: item.type || '', sort_order: item.sort_order || 0 }
  showModal.value = true
}
