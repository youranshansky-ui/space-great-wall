<template>
  <div>
    <div class="flex items-center justify-between mb-8">
      <h1 class="text-2xl font-display font-bold text-hologram-ice">星际联合章节</h1>
...
const API = '/api/admin/alliance-chapters'
...
const form = ref({ chapter_num: 0, title: '', desc: '', content: '' })
...
function openCreate() {
  ...
  form.value = { chapter_num: 0, title: '', desc: '', content: '' }
  showModal.value = true
}

function openEdit(item) {
  editingItem.value = item
  form.value = { chapter_num: item.chapter_num || 0, title: item.title || '', desc: item.desc || '', content: item.content || '' }
  showModal.value = true
}
