<template>
  <div>
    <div class="flex items-center justify-between mb-8">
      <h1 class="text-2xl font-display font-bold text-hologram-ice">作文竞赛</h1>
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="item in list" :key="item.id" class="border-b border-hologram-ice/[0.03] hover:bg-white/[0.01]">
            ...
            <td class="px-4 py-3 text-hologram-ice/50">{{ item.author }}</td>
            <td class="px-4 py-3 text-hologram-ice/50">{{ item.track }}</td>
            <td class="px-4 py-3 text-hologram-ice/50 font-mono text-xs">{{ item.likes }}</td>
            <td class="px-4 py-3">
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60" @click.self="showModal = false">
      <div class="w-full max-w-2xl max-h-[85vh] overflow-y-auto bg-deep-space-900 border border-hologram-ice/[0.1] rounded-2xl p-6 mx-4">
        ...
        <form @submit.prevent="save" class="space-y-4">
          ...
          <div>
            <label class="block text-xs text-hologram-ice/30 mb-2">作者</label>
            <input v-model="form.author" type="text" class="w-full px-4 py-2.5 rounded-lg bg-deep-space-800/60 border border-hologram-ice/[0.1] text-hologram-ice text-sm outline-none focus:border-stellar-gold/30 transition-all" />
          </div>
          <div>
            <label class="block text-xs text-hologram-ice/30 mb-2">学校</label>
            <input v-model="form.school" type="text" class="w-full px-4 py-2.5 rounded-lg bg-deep-space-800/60 border border-hologram-ice/[0.1] text-hologram-ice text-sm outline-none focus:border-stellar-gold/30 transition-all" />
          </div>
          <div>
            <label class="block text-xs text-hologram-ice/30 mb-2">赛道</label>
            <input v-model="form.track" type="text" class="w-full px-4 py-2.5 rounded-lg bg-deep-space-800/60 border border-hologram-ice/[0.1] text-hologram-ice text-sm outline-none focus:border-stellar-gold/30 transition-all" />
          </div>
          <div>
          ...
          <div>
            <label class="block text-xs text-hologram-ice/30 mb-2">点赞数</label>
            <input v-model.number="form.likes" type="number" class="w-full px-4 py-2.5 rounded-lg bg-deep-space-800/60 border border-hologram-ice/[0.1] text-hologram-ice text-sm outline-none focus:border-stellar-gold/30 transition-all" />
          </div>
          <div>
...
const form = ref({ title: '', author: '', school: '', track: '', content: '', image: '', likes: 0, sort_order: 0 })
...
function openCreate() {
  ...
  form.value = { title: '', author: '', school: '', track: '', content: '', image: '', likes: 0, sort_order: 0 }
  showModal.value = true
}

function openEdit(item) {
  editingItem.value = item
  form.value = { title: item.title || '', author: item.author || '', school: item.school || '', track: item.track || '', content: item.content || '', image: item.image || '', likes: item.likes || 0, sort_order: item.sort_order || 0 }
  showModal.value = true
}
