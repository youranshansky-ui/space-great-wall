  ...
  <div class="page-container">
    <div class="py-20">
      <div class="max-w-3xl mx-auto px-6 lg:px-8">
        <!-- 返回链接 -->
        <NuxtLink to="/story#alliance" class="inline-flex items-center gap-2 text-sm text-stellar-warm/60 hover:text-stellar-warm transition-colors mb-10">
        ...
        <div class="flex items-center justify-between">
          <NuxtLink
            v-if="prevChapter"
            :to="`/alliance-chapter-${prevChapter.id}`"
          ...
          <NuxtLink to="/story#alliance" class="text-base text-stellar-warm/60 hover:text-stellar-warm transition-colors">
            返回目录
          </NuxtLink>

          <NuxtLink
            v-if="nextChapter"
            :to="`/alliance-chapter-${nextChapter.id}`"
            class="flex items-center gap-2 text-base text-stellar-warm/60 hover:text-stellar-warm transition-colors group"
          >
...
<script setup>
import { allianceChapters } from '~/data/chapters'
...
const chapter = computed(() => {
  return allianceChapters.find(c => c.id === chapterId.value) || allianceChapters[0]
})

const prevChapter = computed(() => allianceChapters.find(c => c.id === chapterId.value - 1) || null)
const nextChapter = computed(() => allianceChapters.find(c => c.id === chapterId.value + 1) || null)
</script>
